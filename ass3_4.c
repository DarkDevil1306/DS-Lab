/*4. In road traffic maintenance we know the traffic signal is used. Depending on the
real time traffic a particular signal is made on/off for a few seconds. If we 
have the traffic data available then write a program in C to manage the traffic signal.
